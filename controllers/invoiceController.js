...
exports.getInvoices = async (req, res) => {
  const { page=1, limit=20, status, search, customerId, startDate, endDate, sort='-createdAt' } = req.query;
  ...
  paginated(res, invoices, {
    page: +page, limit: +limit, total, pages: Math.ceil(total/limit),
  });
};
...
exports.createInvoice = async (req, res) => {
  ...
  const discountAmount = rest.discountType === 'percent'
...
exports.getInvoice = async (req, res) => {
  const invoice = await Invoice.findOne({ _id: req.params.id, company: req.companyId })
    .populate('customer')
    .populate({ path: 'company', select: 'name email address logo branding bankDetails invoiceSettings' });
...
exports.updateInvoice = async (req, res) => {
  const invoice = await Invoice.findOne({ _id: req.params.id, company: req.companyId });
  if (!invoice) throw new ApiError('Invoice not found', 404);
  if (invoice.status === 'paid') throw new ApiError('Cannot edit a paid invoice', 400);
...
exports.deleteInvoice = async (req, res) => {
  ...
  await Subscription.findOneAndUpdate({ company: req.companyId }, { $inc: { invoiceCount: -1 } });
...
exports.duplicateInvoice = async (req, res) => {
  const original = await Invoice.findOne({ _id: req.params.id, company: req.companyId }).lean();
  if (!original) throw new ApiError('Invoice not found', 404);

  const { _id, invoiceNumber, publicToken, emailSentAt, paidAt, viewedAt, createdAt, updatedAt, ...rest } = original;
  const newNumber = await generateInvoiceNumber(req.companyId);
  const duplicate = await Invoice.create({
    ...rest, invoiceNumber: newNumber, status: 'draft',
    publicToken: uuidv4(), issueDate: new Date(),
  });
